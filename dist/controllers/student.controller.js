...
const studentController = {
  ...
  async getStudent(req, res) {
    try {
      ...
      const user = await _student.default.findOne({
        where: {
  ...
  async grade(req, res) {
    try {
      ...
      return res.json((0, _utils.Response)(201, "graded successfully", data));
    } catch (error) {
      console.log(error);
      return res.status(500).json((0, _utils.Response)(500, "Internal Server Error", error));
    }
  ...
  async updateGrade(req, res) {
    try {
      ...
      const findGrade = await _grade.default.findByPk(id);

      if (findGrade) {
        const data = await _grade.default.update({
          grade,
          graded_by,
          phase
        }, {
      ...
      return res.json((0, _utils.Response)(201, "grade not found"));
    } catch (error) {
      console.log(error);
      return res.status(500).json((0, _utils.Response)(500, "Internal Server Error", error));
    }
