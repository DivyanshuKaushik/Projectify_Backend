...
async function addDummyDataToDB() {
  try {
    const faculties = await Promise.all(_faculty.default.map(async faculty => {
      let pass = await (0, _.hash)(faculty.password);
      return _objectSpread(_objectSpread({}, faculty), {}, {
        password: pass
      });
    }));
    await _faculty2.default.bulkCreate(faculties);
    ...
    await _student.default.bulkCreate(_students.default); // await FacultyAdviser.bulkCreate(facultyAdviserData);
